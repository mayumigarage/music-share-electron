...
- Windows 10/11 または macOS（開発・実行ともに対応）
...
`yt-dlp` は Node.js の依存パッケージではなく、OS のコマンドとしてインストールします。

Windows では PowerShell を開き、次のいずれかで導入してください。
...
macOS では Terminal を開き、Homebrew で導入してください。
...
> Windows のパッケージ版には `tools/yt-dlp.exe` を同梱します。macOS のパッケージ版は `/opt/homebrew/bin/yt-dlp`、`/usr/local/bin/yt-dlp`、PATH の順に `yt-dlp` を探します。
...
| `npm run dist` | `npm run dist:win` と同じ |
| `npm run dist:win` | electron-builder で Windows インストーラー（`.exe`）を生成 |
| `npm run dist:mac` | electron-builder で macOS アプリ（`.dmg` / `.zip`）を生成 |
...
- `MusicShare-1.0.0.dmg` — macOS ディスクイメージ
- `MusicShare-1.0.0-mac.zip` — macOS ZIP

> macOS の `.dmg` / `.zip` は macOS 上で生成してください。Windows から macOS 向け成果物を作るクロスビルドは electron-builder の制約で失敗する場合があります。
